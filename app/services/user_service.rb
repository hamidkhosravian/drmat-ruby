class UserService
  def access(params, request)
    ...
    raise ServerError, 'sorry we have a problem right now' if response.try(:[], 'return').try(:[], 'status').to_i == 200
  ...
  def access_authorize(params,  request)
    ...
    raise AuthError, 'this token is not valid.' unless device || (device.verify_sent + ENV['VERIFY_TTL'] <= Time.now)
