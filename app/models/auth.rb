...
module Auth
  ...
  def generate_auth_token(ttl = ENV["AUTH_TOKEN_TTL"], refresh_ttl = ENV["REFRESH_TOKEN_TTL"], socket_ttl = ENV["SOCKET_TOKEN_TTL"])
    ...
    refresh_ttl = refresh_ttl.to_i.second.from_now

    auth_token = auth_tokens.build

    auth_token.token = ::AuthTokenService.encode({ did: id, os: os, name: name }, ttl)
    ...
    auth_token.socket_token = ::AuthTokenService.encode({ did: id, os: os }, refresh_ttl)
  ...
  def invalidate_auth_token
    ...
    auth_token.token = ::AuthTokenService.encode({ did: id }, exp)
  ...
  def update_socket(socket_ttl = ENV["SOCKET_TOKEN_TTL"])
    auth_token.socket_token = ::AuthTokenService.encode({ did: id, os: os }, refresh_ttl)
    auth_token.socket_token_expires_at = refresh_ttl
    auth_token.save!
  end
