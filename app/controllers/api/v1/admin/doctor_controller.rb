module Api
  module V1
    module Admin
      class DoctorController < AdminController
        before_action :authenticate_user_from_token!

        def create
          param! :phone, String, blank: false, required: true

          @user = AdminUserService.new.access(params, request, 'doctor')
        ...
        def show
          ...
          @profile = User.where(role: 'doctor').find_by(uuid: params['uuid']).profile
          raise ProfileNotFoundError, I18n.t('messages.profile.not_found') unless @profile
          render 'api/v1/admin/profile/doctor'
        end

        def search
          param! :search, String, blank: false, required: true
          @profiles = User.joins(:profile).where(role: 'doctor')
        ...
        def change_role
          ...
          user = User.where(role: 'doctor').find_by(uuid: params['uuid'])
